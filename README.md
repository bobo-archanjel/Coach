...
- **2026-08-28** — Klientsky portál `/portal` napojený na Supabase + auth guard (impeccable, prierezová Fáza B–D): nová migrácia `0002_workout_portal.sql` (`workout_plans` → `workout_days` → `workout_exercises`, `workout_logs`, `coach_notes` + RLS scoped na klienta cez `is_own_client()` / `is_trainer_of_client()`). Obrazovka „Dnes" už nečíta mock — `lib/portal/data.ts` skladá dnešnú session, týždenný pás aj sériu z reálnych záznamov (`workout_logs`, zóna `Europe/Bratislava`). Auth guard v `app/portal/layout.tsx`: bez session → `/prihlasenie`, tréner → `/dashboard`. Prázdne/chybové stavy: neprepojený klient / bez plánu / chyba načítania (`Notice`, zdieľaný vizuál s `ComingSoon`). Nový týždenný stav `missed` (dutý dim-coral obrys). Demo dáta `supabase/seed/0001_portal_demo.sql`; `?preview=` na náhľad stavov bez DB (mimo produkcie). `lib/mock/portal.ts` zmazaný, typy v `lib/portal/types.ts`. Pás „Séria" nesie informáciu (odcvičené / voľno / vynechané, nie jednoliaty coral); chybový stav má `role="alert"` + fokus na retry. **Kontrast `.btn-primary` vyriešený:** plné tlačidlá (landing/auth/portál) dostali pozadie `--iron-red-deep` `#c3341f` (Warm Paper AA 4.78:1), `--iron-red` zostáva pre accenty. Prešlo impeccable finish review (6 fixov, verdikt ship). `DESIGN.md` aktualizované. Dočasne: `DEV_OPEN` v `app/portal/layout.tsx` púšťa `next dev` bez session na portál (kým nie sú reálne Supabase kľúče); v produkcii je guard nepodmienený.
