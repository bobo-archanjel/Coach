...
- Klientsky portál: „Dnes" číta reálne Supabase dáta (viď Surfaces). Chýba odklikávanie cvikov (`completedCount` je 0 mimo stavu `done`), zapisovanie odcvičených hodnôt (`workout_logs.entries`), spárovanie klienta cez pozývací kód (prepojenie `clients.user_id` zatiaľ robí len seed / manuálne) a taby Tréning/Strava/Chat/Profil (fázy B/C/D). `DEV_OPEN` v `layout.tsx` je dočasná pomôcka pre lokál bez reálnych kľúčov — odstrániť po napojení.
