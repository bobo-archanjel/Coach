...
Tréningy sekcia je naostro. Zoznam plánov (`/dashboard/treningy`) v pôvodnom karta-štýle; detail plánu (`/dashboard/treningy/[planId]`) prešiel druhou iteráciou po spätnej väzbe, že pôvodná forma-na-forme pôsobila neprofesionálne — nahradený **split-view builderom** (knižnica + plátno, zvolené z 3 predložených štruktúr): vľavo sticky knižnica cvikov s vyhľadávaním (klik na cvik = okamžité pridanie do aktívneho dňa s defaultmi 3×10/pauza 90s, rovno pripravené na inline úpravu), vpravo dni ako tabs (aktívny stav vizuálne "zliaty" so panelom pod ním) a zoznam cvikov s inline edit/delete (ceruzka/kôš ikony, `.editForm` prepína riadok priamo na formulár bez modálu). Hore v paneli aktívneho dňa je `DayWeekdaySelect` — ktorý deň v týždni sa cvičí (potrebné pre portál "Dnes", auto-submit pri zmene). Vlastné vizuálne triedy v `builder.module.css` (samostatný modul pre tento povrch).
...
- Tréningový builder trénera (`0002_workout_builder`, vetva `feature/trener`) a klientský portál (`0003_portal_client`, `feature/client-side`) vznikli paralelne. Portál sa prispôsobil builderu (jeho `workout_plans`/`workout_days`). ~~Follow-up pre builder: nastavovať `workout_days.weekday`~~ **Vyriešené 2026-08-28:** reálny klient s 4 nastavenými tréningami mal v portáli natrvalo "Dnes máš voľno", lebo builder `weekday` nikdy nenastavoval. Pridané: `AddDayInline` má pri vytváraní nového dňa select "Deň v týždni"; `DayWeekdaySelect` (nová komponenta, hore v paneli aktívneho dňa) umožňuje nastaviť/zmeniť `weekday` aj na existujúcich dňoch — auto-submit pri zmene, nová Server Action `setDayWeekdayAction`. Ideálne `is_active` na pláne ostáva otvorené (zatiaľ "aktívny" = najnovší plán klienta).
