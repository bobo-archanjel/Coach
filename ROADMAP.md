...
**Zdokumentované, neopravené (nízka závažnosť):** `notFound()` na routách s `loading.tsx` (napr. `/dashboard/klienti/x` pre neexistujúceho klienta) vracia HTTP 200 namiesto 404 — streamovaná odpoveď commitne status hlavičku skôr, než sa `notFound()` vnútri Suspense hranice vyrieši (známe obmedzenie Next.js App Router streamingu). Vizuálne správne (ukáže sa naozaj 404 stránka), len status kód je technicky nesprávny — vadí to nástrojom/crawlerom/monitoringu, nie ľuďom. Oprava (odstrániť `loading.tsx` z tejto routy, alebo presunúť existence-check do `generateMetadata`) má UX trade-off (stratí sa skeleton počas načítania) — netriviálne rozhodnutie, necháva sa na neskôr.
