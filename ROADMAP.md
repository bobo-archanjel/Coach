...
**Číslovanie migrácií — ďalšie voľné číslo je `0035`.** (`0030`–`0034` sú v `dev`, `0031`–`0034` potvrdené bežiace aj na cloud DB.) Dohodnite si vopred, kto berie ktoré číslo, nech sa nezraziť dva rovnaké súbory na dvoch vetvách:
...
| 0031 | `exercise_equipment.sql` (`exercises.equipment text` — štruktúrovaný filter vybavenia pre AI generátor, `feature/ai-plan-zameranie`, zmergované 2026-09-07) — **stĺpec beží na cloud DB, ale `scripts/import-exercises.mjs` re-import sa ešte nespustil (0/888 cvikov má equipment) → filter je stále v tichom fallbacku, viď poznámka nižšie** | Tréner |
