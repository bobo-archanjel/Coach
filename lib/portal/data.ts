...
export async function getPortalData(): Promise<PortalResult> {
  try {
    ...
    // published: false = tréner ešte plán len rozostavuje (0016) — dovtedy sa
    ...
    if (activeDayOverride) {
      ...
      // "Začať tréning" (clients.active_day_id, RPC set_active_plan, 0017) — má
...
export async function getPortalTraining(): Promise<PortalTrainingResult> {
  try {
    ...
    // published: false = trénerov koncept (0016) — v klientovom zozname sa nezobrazí,
